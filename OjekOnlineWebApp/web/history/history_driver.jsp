...
<table border="0" width="100%" >
    <%
        ...
        String query = String.format("SELECT * FROM pesanan WHERE user_driver = \"%s\" AND is_hidden_driver = FALSE" , username);
        ...
        while((!responsehistory.isEmpty())&&(i<responsehistory.size())) {
            ...
            int hidden = history.getIsHiddenDriver();
            if(hidden==0){
                ...
                String queryname = String.format("select name from customer where username = \"%s\"" , usercustomer);
                String name = port.searchCustomerName(queryname);
                out.print(name);
                out.println("</div>\n" +
                        "            </div>\n" +
                        "            <div class=\"header-row-button\">");
                out.println("<form action=\"hide_driver.jsp\" method=\"post\">");
                ...
                out.println("Gave <span class=\"rating\">"+rating+"</span> stars for this order");
                out.println("</div>\n" +
                        "        <div>\n" +
                        "            and left comment:\n" +
                        "        </div>\n" +
